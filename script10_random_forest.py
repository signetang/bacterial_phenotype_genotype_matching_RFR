...
def eval_permuted(data_set, trained_model, genomic_matrix, output_file):
    ...
    print '0-hypothesis: "The model predicts equally well when the genetic values are randomly permuted for each feature."'
    print 'Alternative hypothesis: "The model predicts better than when the genetic values are randomly permuted for each feature."'
    print 'If the Pearson Correlation of',PC,'is above the interval [',np.mean(PC_list)-2*np.std(PC_list), ',', np.mean(PC_list)+2*np.std(PC_list),'], then the 0-hypothesis can be rejected on a 2.5% significance level.'
    print 'If the Root Mean Square Error of',RMSE,'is below the interval [',np.mean(RMSE_list)-2*np.std(RMSE_list), ',', np.mean(RMSE_list)+2*np.std(RMSE_list),'], then the 0-hypothesis can be rejected on a 2.5% significance level.'
    print 'If the Explained Variance score of',EV,'is above the interval [', np.mean(EV_list)-2*np.std(EV_list), ',', np.mean(EV_list)+2*np.std(EV_list), '],  then the 0-hypothesis can be rejeced on a 2.5% significance level.'
    ...
    print '0-hypothesis: "The model predicts equally well when the genetic values are randomly switched between strains."'
    print 'Alternative hypothesis: "The model predicts better than when the genetic values are randomly switched between strains."'
    print 'If the Pearson Correlation of',PC,'is above the interval [',np.mean(PC_list)-2*np.std(PC_list), ',', np.mean(PC_list)+2*np.std(PC_list),'], then the 0-hypothesis can be rejected on a 2.5% significance level.'
    print 'If the Root Mean Square Error of',RMSE,'is below the interval [',np.mean(RMSE_list)-2*np.std(RMSE_list), ',', np.mean(RMSE_list)+2*np.std(RMSE_list),'], then the 0-hypothesis can be rejected on a 2.5% significance level.'
    print 'If the Explained Variance score of',EV,'is above the interval [', np.mean(EV_list)-2*np.std(EV_list), ',', np.mean(EV_list)+2*np.std(EV_list), '],  then the 0-hypothesis can be rejeced on a 2.5% significance level.'
...
def feature_importance(data_set, trained_model, output_prefix, non_genetic_features, name_dictionary=None, method='impurity_decrease', score='EV', n_features=30, iterations_per_feature=10):
    ...
    if name_dictionary != None:
        ...
        variables = np.array([var+': '+','.join([name_dictionary[v] for v in var.split(',')]) if var not in non_genetic_features else var for var in variables])
...
def shapImportances(test_data, trained_model, output_prefix, non_genetic_features, name_dictionary=None, n_features=40, individual_sample_plots=False):
...
def gene_dict(pa_file):
    ...
    gene_names = pd.read_csv(pa_file, sep=',', dtype='str', usecols=[0,2])
    for i in range(len(gene_names)):
        gene_dict[gene_names.iloc[i,0]] = gene_names.iloc[i,1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--genomic_matrix', help='The gene or Pfam presence/absence matrix or the Pfam count matrix. Fisrt column must be the "Strains" ID.')
    ...
    #minimal_model = ['Strains','Value','Measurement','Volume','Temp','Yeast','Salt','Taxonomy','PF00639.16','PF02502.13','PF02669.10','PF02254.13','PF03382.9','PF13450.1','PF12730.2','PF01609.16','PF01656.18']
    #train_set, test_set, val_set = train_set.loc[:,minimal_model], test_set.loc[:,minimal_model], val_set.loc[:,minimal_model]
    #train_set, test_set, val_set = train_set.loc[train_set['Taxonomy']=='Lactococcus lactis subsp. cremoris'], test_set.loc[test_set['Taxonomy']=='Lactococcus lactis subsp. cremoris'], val_set.loc[val_set['Taxonomy']=='Lactococcus lactis subsp. cremoris']
    #train_set, test_set, val_set = train_set.loc[train_set['Temp']=='40'], test_set.loc[test_set['Temp']=='40'], val_set.loc[val_set['Temp']=='40']
    ...
    #criterion = ['mse', 'mae']
    #bootstrap = [True, False] #True is the default and should be used if max_features='auto'
    param_grid = {  'random_state': [0,6,12,18,24,30,36,random_seed],\
    ...
    n_iter = 30 #iterations of the randomized grid-search
    ...
    #best_param_model = model.set_params(**{'n_estimators':200, 'min_samples_leaf':6}) #faster than getting the best params. Only for tests.
    test_model = False
    ...
    if args.genomic_matrix[-25:] in ['pruned_pfam_pa_matrix.txt', 'ned_pfam_count_matrix.txt']:
        ...
        name_dictionary = gene_dict('../data/roary/gene_presence_absence.csv')


    print '\nFinding the most important features with Shap...'
    shapImportances(test_set, trained_model, args.output_prefix, non_genetic_features, individual_sample_plots=False)
    #print '\nFinding the most important features with impurity decrease...'
    #feature_importance(test_set, trained_model, args.output_prefix, non_genetic_features=non_genetic_features, method='impurity_decrease')
    ...
    #print '\nFinding the most important features with iterative permutation strategy...'
    #iterativePermutation(train_set, test_set, trained_model, n_iterations=n_iterations, non_genetic_features=non_genetic_features, outputfile=args.output_prefix+'_iterative_permutation_feature_selection.txt', name_dictionary=name_dictionary, features_to_remove=features_to_remove, score=score,iterations_per_feature=iterations_per_feature)


    #shapImportances(test_set.loc[test_set['Taxonomy']=='Lactococcus lactis subsp. lactis'], trained_model, args.output_prefix+'_lactis', non_genetic_features, individual_sample_plots=False)
    #shapImportances(test_set.loc[test_set['Taxonomy']=='Lactococcus lactis subsp. cremoris'], trained_model, args.output_prefix+'_cremoris', non_genetic_features, individual_sample_plots=False)
